...
const STATE_META = {
  ...
  cancelled:   { icon: XCircle,       label: 'Cancelado',   color: 'text-white/30',    spin: false },
  ...
  return (
    <div className="flex items-center gap-3 px-4 py-3 hover:bg-white/[0.03] transition-colors group">
      ...
      <div className="flex-1 min-w-0">
        <p className="text-sm font-medium text-white/85 truncate" title={item.filename}>{item.filename || 'Descarga'}</p>
        <div className="flex items-center gap-2 mt-0.5">
          ...
          <span className="text-[11px] text-white/25">·</span>
          <span className="text-[11px] text-white/30">{formatRelativeTime(item.startedAt)}</span>
        </div>
      ...
      >
        <div className="flex items-center justify-between px-4 py-3.5 border-b border-white/[0.06]">
          <div className="flex items-center gap-2.5">
            ...
            <h2 className="text-sm font-semibold text-white/85">Descargas</h2>
            {items.length > 0 && (
              <span className="text-[10px] font-semibold px-1.5 py-0.5 rounded-full bg-white/[0.06] text-white/40">{items.length}</span>
            )}
          </div>
