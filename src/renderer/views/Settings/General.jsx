...
export default function General() {
  ...
  const { soundPrefs, setSoundPrefs, playSound } = useSound();
  ...
  return (
    <div className="flex flex-col gap-6">
      <div>
        <h2 className="text-base font-semibold text-fg mb-0.5">{t('set_general')}</h2>
        <p className="text-sm text-fg/35">Preferencias generales de AppSpawner.</p>
      </div>
            ...
            <button
              ...
              className={`
                ...
                ${language === lang
                  ...
                  : 'bg-overlay/[0.04] text-fg/50 hover:text-fg/75 border border-line/[0.06]'
                }
              `}
            >
      ...
      <Section title="Mantenimiento">
        <div className="glass rounded-xl p-4 flex flex-col gap-4">
          <div className="flex items-center justify-between gap-4">
            <div>
              <p className="text-sm font-medium text-fg/75">Reparar accesos directos</p>
              <p className="text-xs text-fg/35 mt-0.5">Regenera accesos con iconos, rutas y nombres actuales.</p>
            </div>
            <button
          ...
          <div className="flex items-center justify-between gap-4">
            <div>
              <p className="text-sm font-medium text-fg/75">Ver presentación inicial</p>
              <p className="text-xs text-fg/35 mt-0.5">Reabre el tour de AppSpawner sin cambiar tus datos.</p>
            </div>
      ...
      {/* ── Sonidos ───────────────────────────────────────────────────────── */}
      <Section title="Sonidos">
        <div className="glass rounded-xl p-4 flex flex-col gap-4">
          <Switch
            checked={soundPrefs.enabled}
            onChange={v => { setSoundPrefs({ enabled: v }); if (v) setTimeout(() => playSound('toggleOn'), 80); }}
            label="Sonidos de interfaz"
            description="Tonos sutiles al lanzar apps, marcar favoritos, notificaciones y más."
          />
          {soundPrefs.enabled && (
            <>
              <div className="divider" />
              <div className="flex flex-col gap-2">
                <div className="flex items-center justify-between">
                  <span className="text-sm font-medium text-fg/75">Volumen</span>
                  <span className="text-[11px] text-fg/40 tabular-nums">{soundPrefs.volume}%</span>
                </div>
                <input
                  type="range"
                  min={0}
      ...
      <div className="flex items-start gap-2.5 glass rounded-xl px-4 py-3">
        ...
        <p className="text-xs text-fg/40 leading-relaxed">
          AppSpawner vive en el <strong className="text-fg/60">System Tray</strong> (bandeja del sistema).
          Puedes acceder rápidamente a todas tus apps sin abrir el Dashboard.
          Atajo global: <kbd className="px-1.5 py-0.5 bg-overlay/[0.07] rounded text-[10px]">Ctrl+Alt+Space</kbd>
        </p>
      </div>
...
function Section({ title, children }) {
  return (
    <div className="flex flex-col gap-2.5">
      <h3 className="text-[11px] font-semibold text-fg/35 uppercase tracking-wider">{title}</h3>
      {children}
    </div>
  );
}
